...
from scripts.constants import MULTICLASS_ML_ENSEMBLE, MULTICLASS_EMOTION_LYRIC_PATH, MULTICLASS_BASE_ML_MODELS
from scripts.utils import choose_best_ml_models
...
def fetch_base_model_list():
    return {
        "lr": LogisticRegression(max_iter=3000, class_weight="balanced", C=1.0),
        "svm": CalibratedClassifierCV(LinearSVC(class_weight="balanced", max_iter=10000, C=0.7, random_state=100)),
...
def build_stacking_model():
    base_estimators = [
        ...
        # ("nb", MultinomialNB(alpha=0.3)),
        # ("rf", RandomForestClassifier(n_estimators=50, n_jobs=-1)),
        ("extra_trees", ExtraTreesClassifier(n_estimators=100, n_jobs=-1, class_weight="balanced", random_state=123, max_depth=40, min_samples_leaf=3, min_samples_split=5)),
        # ("dt", DecisionTreeClassifier(max_depth=20)),
        # ("knn", KNeighborsClassifier(n_neighbors=7)),
    ]
...
def evaluate_multilabel(Y_true, y_prob):
    ...
    for c, l in enumerate(PLUTCHIK_EMOTIONS):
        Y_pred[:, c] = (y_prob[:, c] >= LABEL_THRESHOLDS[l]).astype(int)
...
if __name__ == "__main__":
   ...
   # feature extraction
    ...
    print("\n---- Evaluating & Freezing Individual Models ----")
    # base_learners = fetch_base_model_list()
    # base_model_scores = choose_best_ml_models(base_learners, X_train, y_train, MULTICLASS_BASE_ML_MODELS)
    ...
    evaluate_multilabel(Y_true=y_test, y_prob=Y_prob)
